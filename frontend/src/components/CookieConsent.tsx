...
export default function CookieConsent() {
  ...
  useEffect(() => {
    const consent = localStorage.getItem('shopflow-cookie-consent');
    if (!consent) {
  ...
  const handleAccept = () => {
    localStorage.setItem('shopflow-cookie-consent', 'true');
    setShow(false);
  };
    ...
    <>
      {show && (
        <div className="anim-slide-up fixed bottom-0 left-0 right-0 z-50 p-4 sm:p-6">
          <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 rounded-3xl border border-border bg-surface/95 p-4 backdrop-blur-md shadow-2xl sm:flex-row sm:p-6">
            <div className="flex-1 text-sm text-secondary text-center sm:text-left">
              ...
              <Link to="/privacy" className="font-semibold text-accent-secondary hover:underline">
            ...
            <div className="flex w-full shrink-0 gap-3 sm:w-auto">
              <button
                ...
                className="flex-1 whitespace-nowrap rounded-full bg-accent px-6 py-2.5 text-sm font-bold text-primary transition-all hover:opacity-90 active:scale-95 sm:flex-none"
              >
                {t('cookies.accept', { defaultValue: 'Accept All' })}
              </button>
              <button
                onClick={() => setShow(false)}
                className="flex items-center justify-center rounded-full border border-border p-2.5 text-secondary hover:bg-background hover:text-primary active:scale-95 sm:hidden"
                aria-label="Close"
              >
                <X className="h-5 w-5" />
              </button>
