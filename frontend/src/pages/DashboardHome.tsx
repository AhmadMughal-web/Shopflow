...
import { AlertTriangle, Lock, Loader2, X, Mail, ShieldCheck, Package } from 'lucide-react';
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-border bg-surface p-4 sm:p-5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 font-bold capitalize">{t(`dashboard.role${roleKey}`, { defaultValue: user?.effective_role || 'customer' })}</p>
          </div>
        </div>
        <div className="flex items-start gap-3 rounded-2xl border border-border bg-surface p-4 sm:p-5">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent-secondary/15 text-accent-secondary">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 font-bold">{t('dashboard.ordersConnected', { defaultValue: 'Orders sync automatically.' })}</p>
          </div>
        </div>
      </div>
          ...
          >
            ...
            {deleteStep === 'confirm' ? (
              <>
                ...
                {deleteError && (
                  <p className="mb-4 rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700">{deleteError}</p>
                )}

                <div className="flex gap-3">
                  <button
                    onClick={closeDeleteModal}
                    className="flex-1 rounded-full border border-border bg-background py-2.5 text-sm font-semibold text-primary hover:bg-muted transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-full bg-red-600 py-2.5 text-sm font-semibold text-white hover:bg-red-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
              ...
              <>
                ...
                <input
                  ...
                  placeholder="••••••"
                  className="mb-6 w-full rounded-2xl border border-border bg-background px-4 py-3.5 text-center text-2xl font-mono tracking-[0.5em] outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all"
                  autoFocus
                />
